...
OV_DLLFNCEXPORT UA_StatusCode propertyValueStatementOPCUAInterface_interface_ovVisibilityNodeToOPCUA(
	...
	UA_VisibilityEnum tmpVisibility = 0;
	do {
		ov_element_getnextpart(&tmpParrent, &tmpPart, OV_ET_VARIABLE);
		if (tmpPart.elemtype == OV_ET_NONE)
			break;
		if (ov_string_compare(tmpPart.elemunion.pvar->v_identifier, "VisibilityEnum") == OV_STRCMP_EQUAL){
			tmpVisibility = *(UA_UInt32*)tmpPart.pvalue;
			break;
		}
	} while(TRUE);


	((UA_Variant*)&((UA_VariableNode*)newNode)->value.data.value.value)->type = &UA_PROPERTYVALUESTATEMENT[UA_PROPERTYVALUESTATEMENT_VISIBILITYENUM];
	((UA_Variant*)&((UA_VariableNode*)newNode)->value.data.value.value)->data = UA_VisibilityEnum_new();
	...
	UA_VisibilityEnum_copy(&tmpVisibility, ((UA_Variant*)&((UA_VariableNode*)newNode)->value.data.value.value)->data);
	UA_VisibilityEnum_deleteMembers(&tmpVisibility);
	...
	// dataType
	((UA_VariableNode*)newNode)->dataType = UA_NODEID_NUMERIC(pinterface->v_modelnamespace.index, UA_NSPROPERTYVALUESTATEMENTID_VISIBILITYENUM);
