...
#ifndef OV_COMPILE_LIBRARY_subModelSendMessageOPCUAInterface
#define OV_COMPILE_LIBRARY_subModelSendMessageOPCUAInterface
#endif
...
static const UA_Node * OV_NodeStore_getNode(void *handle, const UA_NodeId *nodeId){
	...
	if(Ov_CanCastTo(subModelSendMessage_SubModelSendMessage, pobj)){
		if (openaasOPCUAInterface_interface_ovSubModelNodeToOPCUA(NULL, nodeId, &opcuaNode) == UA_STATUSCODE_GOOD)
			tmpNode = opcuaNode;
	}else if(Ov_CanCastTo(subModelSendMessage_SendMessageService, pobj)){
...
UA_NodestoreInterface* subModelSendMessageOPCUAInterface_interface_ovNodeStoreInterfaceSubModelSendMessageNew(void) {
...
void subModelSendMessageOPCUAInterface_interface_ovNodeStoreInterfaceSubModelSendMessageDelete(UA_NodestoreInterface * nsi){
	if (nsi->handle)
		UA_free(nsi->handle);
}
