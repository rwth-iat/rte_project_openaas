    ...
    OV_INSTPTR_ov_domain pparent = NULL;
   	...
   	if (Ov_CanCastTo(propertyValueStatement_PropertyValueStatementList, pparent)){
   		Ov_ForEachChild(ov_containment, pparent, pchild){
   			if (Ov_CanCastTo(propertyValueStatement_ExpressionSemantic, pchild) && pchild != pobj){
   				ov_logfile_error("%s: cannot instantiate - ExpressionSemantic instance already exists", pinst->v_identifier);
   				return OV_ERR_ALREADYEXISTS;
