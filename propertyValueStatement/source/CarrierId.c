    ...
    */
	...
	if (Ov_CanCastTo(propertyValueStatement_PropertyValueStatementList, pparent)){
		Ov_ForEachChild(ov_containment, pparent, pchild){
			if (Ov_CanCastTo(propertyValueStatement_CarrierId, pchild) && pchild != pobj){
				ov_logfile_error("%s: cannot instantiate - CarrierId instance already exists", pinst->v_identifier);
				return OV_ERR_ALREADYEXISTS;
