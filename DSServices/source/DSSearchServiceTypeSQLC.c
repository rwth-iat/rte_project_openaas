
/******************************************************************************
*
*   FILE
*   ----
*   DSSearchServiceTypeSQLC.c
*
*   History
*   -------
*   2018-07-30   File created
*
*******************************************************************************
*
*   This file is generated by the 'acplt_builder' command
*
******************************************************************************/


#ifndef OV_COMPILE_LIBRARY_DSServices
#define OV_COMPILE_LIBRARY_DSServices
#endif


#include "DSServices.h"
#include "ov_macros.h"
#include "service_helper.h"

struct searchStatement{
	OV_STRING carrierID;
	OV_STRING propertyID;
	OV_STRING expressionSemantic;
	OV_STRING relation;
	OV_STRING value;
	OV_STRING valueType;
	OV_STRING submodel;
};

struct endpoint{
	OV_STRING protocolType;
	OV_STRING endpointString;
};

struct component{
	OV_STRING componentID;
	struct endpoint* endpoints;
	OV_UINT	  endpointsSize;
};

OV_DLLFNCEXPORT OV_RESULT DSServices_DSSearchServiceTypeSQLC_executeService(OV_INSTPTR_openAASDiscoveryServer_DSService pinst, const json_data JsonInput, OV_STRING *JsonOutput, OV_STRING *errorMessage) {
    /*    
    *   local variables
    */
	// Parsing Body
	OV_STRING_VEC componentIDs;
	OV_STRING_VEC* resultStatements = NULL;
	OV_STRING_VEC resultStatements2;
	resultStatements2.veclen = 0;
	resultStatements2.value = NULL;
	componentIDs.value = NULL;
	componentIDs.veclen = 0;
	OV_STRING_VEC endpointStruct;
	endpointStruct.value = NULL;
	endpointStruct.veclen = 0;
	struct component *components = NULL;
	OV_STRING_VEC tags;
	tags.value = NULL;
	tags.veclen = 0;
	Ov_SetDynamicVectorLength(&tags, 4, STRING);
	ov_string_setvalue(&tags.value[0], "componentID");
	ov_string_setvalue(&tags.value[1], "securityKey");
	ov_string_setvalue(&tags.value[2], "statements");
	ov_string_setvalue(&tags.value[3], "responseMode");
	struct searchStatement *searchStatements = NULL;
	OV_UINT searchStatementSize = 0;
	OV_UINT_VEC tokenIndex;
	tokenIndex.value = NULL;
	tokenIndex.veclen = 0;
	Ov_SetDynamicVectorLength(&tokenIndex, 4, UINT);
	OV_STRING responseMode = NULL;

	jsonGetTokenIndexByTags(tags, JsonInput, 1, &tokenIndex);

	OV_STRING componentID = NULL;
	jsonGetValueByToken(JsonInput.js, &JsonInput.token[tokenIndex.value[0]+1], &componentID);
	OV_STRING securityKey = NULL;
	jsonGetValueByToken(JsonInput.js, &JsonInput.token[tokenIndex.value[1]+1], &securityKey);

	// check SecurityKey in Database
	OV_RESULT resultOV = checkSecurityKeySQLC(pinst->v_DBWrapperUsed, componentID, securityKey);
	if (resultOV){
		ov_string_setvalue(errorMessage, "SecurityKey is not correct");
		goto FINALIZE;
	}
	// get statements from JSON
	searchStatementSize = JsonInput.token[tokenIndex.value[2]+1].size;
	searchStatements = malloc(sizeof(struct searchStatement)*searchStatementSize);
	for (OV_UINT i = 0; i < searchStatementSize; i++){
		// value + 2 start of objects + i*15 next object + 2 values of carrierID
		searchStatements[i].carrierID = NULL;
		jsonGetValueByToken(JsonInput.js, &JsonInput.token[tokenIndex.value[2]+2+i*15+2], &searchStatements[i].carrierID);
		// value + 2 start of objects + i*15 next object + 4 values of propertyID
		searchStatements[i].propertyID = NULL;
		jsonGetValueByToken(JsonInput.js, &JsonInput.token[tokenIndex.value[2]+2+i*15+4], &searchStatements[i].propertyID);
		// value + 2 start of objects + i*15 next object + 6 values of expressionSemantic
		searchStatements[i].expressionSemantic = NULL;
		jsonGetValueByToken(JsonInput.js, &JsonInput.token[tokenIndex.value[2]+2+i*15+6], &searchStatements[i].expressionSemantic);
		// value + 2 start of objects + i*15 next object + 8 values of relation
		searchStatements[i].relation = NULL;
		jsonGetValueByToken(JsonInput.js, &JsonInput.token[tokenIndex.value[2]+2+i*15+8], &searchStatements[i].relation);
		// value + 2 start of objects + i*15 next object + 10 values of value
		searchStatements[i].value = NULL;
		jsonGetValueByToken(JsonInput.js, &JsonInput.token[tokenIndex.value[2]+2+i*15+10], &searchStatements[i].value);
		// value + 2 start of objects + i*15 next object + 12 values of valuetype
		searchStatements[i].valueType = NULL;
		// ['','Integer','Real','Text']
		jsonGetValueByToken(JsonInput.js, &JsonInput.token[tokenIndex.value[2]+2+i*15+12], &searchStatements[i].valueType);
		// value + 2 start of objects + i*15 next object + 14 values of submodel
		searchStatements[i].submodel = NULL;
		jsonGetValueByToken(JsonInput.js, &JsonInput.token[tokenIndex.value[2]+2+i*15+14], &searchStatements[i].submodel);
	}

	jsonGetValueByToken(JsonInput.js, &JsonInput.token[tokenIndex.value[3]+1], &responseMode);

	// search for statements in Database and get componentID and endpoints
	OV_UINT queryCount = searchStatementSize;
	struct DB_QUERY * query = NULL;
	query = malloc(sizeof(struct DB_QUERY) * queryCount);
	OV_STRING_VEC table;
	table.value = NULL;
	table.veclen = 0;
	Ov_SetDynamicVectorLength(&table,queryCount, STRING);

	for (OV_UINT i = 0; i < searchStatementSize; i++){
		query[i].column.veclen = 0;
		query[i].column.value = NULL;
		Ov_SetDynamicVectorLength(&query[i].column, query[i].column.veclen + 6, STRING);
		ov_string_setvalue(&query[i].column.value[query[i].column.veclen-6], "CarrierID");
		ov_string_setvalue(&query[i].column.value[query[i].column.veclen-5], "PropertyID");
		ov_string_setvalue(&query[i].column.value[query[i].column.veclen-4], "ExpressionSemantic");
		ov_string_setvalue(&query[i].column.value[query[i].column.veclen-3], "Relation");
		ov_string_setvalue(&query[i].column.value[query[i].column.veclen-2], "Value");
		ov_string_setvalue(&query[i].column.value[query[i].column.veclen-1], "SubModel");
		query[i].value.veclen = 0;
		query[i].value.value = NULL;
		query[i].value_relation.veclen = 0;
		query[i].value_relation.value = NULL;
		query[i].value_optional.veclen = 0;
		query[i].value_optional.value = NULL;
		query[i].value_optional_relation.veclen = 0;
		query[i].value_optional_relation.value = NULL;
		if (ov_string_compare(searchStatements[i].valueType, "Numeric") == OV_STRCMP_EQUAL){
			ov_string_setvalue(&table.value[i], "statements_Numeric");
			if (ov_string_compare(searchStatements[i].relation, "==") == OV_STRCMP_EQUAL || ov_string_compare(searchStatements[i].relation, "!=") == OV_STRCMP_EQUAL){
				Ov_SetDynamicVectorLength(&query[i].value_optional, query[i].value_optional.veclen + 30, STRING);
				Ov_SetDynamicVectorLength(&query[i].value_optional_relation, query[i].value_optional_relation.veclen + 30, STRING);
				ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-30], searchStatements[i].carrierID);
				ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-30], "=");
				ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-29], searchStatements[i].propertyID);
				ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-29], "=");
				ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-28], searchStatements[i].expressionSemantic);
				ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-28], "=");
				ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-25], searchStatements[i].submodel);
				ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-25], "=");

				ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-24], searchStatements[i].carrierID);
				ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-24], "=");
				ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-23], searchStatements[i].propertyID);
				ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-23], "=");
				ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-22], searchStatements[i].expressionSemantic);
				ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-22], "=");
				ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-19], searchStatements[i].submodel);
				ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-19], "=");

				ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-18], searchStatements[i].carrierID);
				ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-18], "=");
				ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-17], searchStatements[i].propertyID);
				ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-17], "=");
				ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-16], searchStatements[i].expressionSemantic);
				ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-16], "=");
				ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-13], searchStatements[i].submodel);
				ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-13], "=");

				ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-12], searchStatements[i].carrierID);
				ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-12], "=");
				ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-11], searchStatements[i].propertyID);
				ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-11], "=");
				ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-10], searchStatements[i].expressionSemantic);
				ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-10], "=");
				ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-7], searchStatements[i].submodel);
				ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-7], "=");

				ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-6], searchStatements[i].carrierID);
				ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-7], "=");
				ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-6], searchStatements[i].carrierID);
				ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-6], "=");
				ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-5], searchStatements[i].propertyID);
				ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-5], "=");
				ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-4], searchStatements[i].expressionSemantic);
				ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-4], "=");
				ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-1], searchStatements[i].submodel);
				ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-1], "=");

				if (ov_string_compare(searchStatements[i].relation, "==") == OV_STRCMP_EQUAL){
					ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-27], "<");
					ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-27], "=");
					ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-26], searchStatements[i].value);
					ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-26], ">");

					ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-21], "<=");
					ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-21], "=");
					ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-20], searchStatements[i].value);
					ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-20], ">=");

					ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-15], "==");
					ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-15], "=");
					ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-14], searchStatements[i].value);
					ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-14], "=");

					ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-9], ">=");
					ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-9], "=");
					ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-8], searchStatements[i].value);
					ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-8], "<=");

					ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-3], ">");
					ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-3], "=");
					ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-2], searchStatements[i].value);
					ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-2], "<");
				}else if (ov_string_compare(searchStatements[i].relation, "!=") == OV_STRCMP_EQUAL){
					ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-27], "<");
					ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-27], "=");
					ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-26], searchStatements[i].value);
					ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-26], "<=");

					ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-21], "<=");
					ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-21], "=");
					ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-20], searchStatements[i].value);
					ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-20], "<");

					ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-15], "!=");
					ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-15], "=");
					ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-14], searchStatements[i].value);
					ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-14], "=");

					ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-9], ">=");
					ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-9], "=");
					ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-8], searchStatements[i].value);
					ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-8], ">");

					ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-3], ">");
					ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-3], "=");
					ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-2], searchStatements[i].value);
					ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-2], ">=");
				}else{
					// TODO: Error handling
				}
			}else{ // <, <=, =>, > => 2 optional_values
				Ov_SetDynamicVectorLength(&query[i].value_optional, query[i].value_optional.veclen + 18, STRING);
				Ov_SetDynamicVectorLength(&query[i].value_optional_relation, query[i].value_optional_relation.veclen + 18, STRING);
				ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-18], searchStatements[i].carrierID);
				ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-18], "=");
				ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-17], searchStatements[i].propertyID);
				ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-17], "=");
				ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-16], searchStatements[i].expressionSemantic);
				ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-16], "=");
				ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-13], searchStatements[i].submodel);
				ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-13], "=");

				ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-12], searchStatements[i].carrierID);
				ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-12], "=");
				ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-11], searchStatements[i].propertyID);
				ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-11], "=");
				ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-10], searchStatements[i].expressionSemantic);
				ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-10], "=");
				ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-7], searchStatements[i].submodel);
				ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-7], "=");

				ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-6], searchStatements[i].carrierID);
				ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-6], "=");
				ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-5], searchStatements[i].propertyID);
				ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-5], "=");
				ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-4], searchStatements[i].expressionSemantic);
				ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-4], "=");
				ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-1], searchStatements[i].submodel);
				ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-1], "=");

				if (ov_string_compare(searchStatements[i].relation, "<") == OV_STRCMP_EQUAL){
					ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-15], "<");
					ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-15], "=");
					ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-14], searchStatements[i].value);
					ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-14], ">=");

					ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-9], "<=");
					ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-9], "=");
					ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-8], searchStatements[i].value);
					ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-8], ">");

					ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-3], "==");
					ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-3], "=");
					ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-2], searchStatements[i].value);
					ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-2], "<");
				}else if (ov_string_compare(searchStatements[i].relation, "<=") == OV_STRCMP_EQUAL){
					ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-15], "<");
					ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-15], "=");
					ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-14], searchStatements[i].value);
					ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-14], ">");

					ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-9], "<=");
					ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-9], "=");
					ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-8], searchStatements[i].value);
					ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-8], ">=");

					ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-3], "==");
					ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-3], "=");
					ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-2], searchStatements[i].value);
					ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-2], "<=");
				}else if (ov_string_compare(searchStatements[i].relation, ">=") == OV_STRCMP_EQUAL){
					ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-15], "==");
					ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-15], "=");
					ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-14], searchStatements[i].value);
					ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-14], ">=");

					ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-9], ">=");
					ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-9], "=");
					ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-8], searchStatements[i].value);
					ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-8], "<=");

					ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-3], ">");
					ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-3], "=");
					ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-2], searchStatements[i].value);
					ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-2], "<");
				}else if (ov_string_compare(searchStatements[i].relation, ">") == OV_STRCMP_EQUAL){
					ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-15], "==");
					ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-15], "=");
					ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-14], searchStatements[i].value);
					ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-14], ">");

					ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-9], ">=");
					ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-9], "=");
					ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-8], searchStatements[i].value);
					ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-8], "<");

					ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-3], ">");
					ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-3], "=");
					ov_string_setvalue(&query[i].value_optional.value[query[i].value_optional.veclen-2], searchStatements[i].value);
					ov_string_setvalue(&query[i].value_optional_relation.value[query[i].value_optional_relation.veclen-2], "<=");
				}else{
					// TODO: Error Handling
				}
			}
		}else{
			ov_string_setvalue(&table.value[i], "statements_TextBoolean");
			Ov_SetDynamicVectorLength(&query[i].value, query[i].value.veclen + 6, STRING);
			Ov_SetDynamicVectorLength(&query[i].value_relation, query[i].value_relation.veclen + 6, STRING);
			ov_string_setvalue(&query[i].value.value[query[i].value.veclen-6], searchStatements[i].carrierID);
			ov_string_setvalue(&query[i].value_relation.value[query[i].value_relation.veclen-6], "=");
			ov_string_setvalue(&query[i].value.value[query[i].value.veclen-5], searchStatements[i].propertyID);
			ov_string_setvalue(&query[i].value_relation.value[query[i].value_relation.veclen-5], "=");
			ov_string_setvalue(&query[i].value.value[query[i].value.veclen-4], searchStatements[i].expressionSemantic);
			ov_string_setvalue(&query[i].value_relation.value[query[i].value_relation.veclen-4], "=");
			ov_string_setvalue(&query[i].value.value[query[i].value.veclen-3], searchStatements[i].relation);
			ov_string_setvalue(&query[i].value_relation.value[query[i].value_relation.veclen-3], "=");
			ov_string_setvalue(&query[i].value.value[query[i].value.veclen-2], searchStatements[i].value);
			ov_string_setvalue(&query[i].value_relation.value[query[i].value_relation.veclen-2], "=");
			ov_string_setvalue(&query[i].value.value[query[i].value.veclen-1], searchStatements[i].submodel);
			ov_string_setvalue(&query[i].value_relation.value[query[i].value_relation.veclen-1], "=");
		}
	}

	OV_INSTPTR_openAASDiscoveryServer_DBWrapper pDBWrapper = NULL;
	OV_VTBLPTR_openAASDiscoveryServer_DBWrapper pDBWrapperVTable = NULL;
	for (OV_UINT i = 0; i < pinst->v_DBWrapperUsed.veclen; i++){
		pDBWrapper = Ov_DynamicPtrCast(openAASDiscoveryServer_DBWrapper, ov_path_getobjectpointer(pinst->v_DBWrapperUsed.value[i], 2));
		if (!pDBWrapper)
			break;

		Ov_GetVTablePtr(openAASDiscoveryServer_DBWrapper,pDBWrapperVTable, pDBWrapper);
		pDBWrapperVTable->m_getComponentID(pDBWrapper, table, query, searchStatementSize, &componentIDs);
		if (ov_string_compare(responseMode, "full") == OV_STRCMP_EQUAL){
			if (componentIDs.veclen > 0){
				resultStatements = malloc(sizeof(OV_STRING_VEC)*componentIDs.veclen);
				for (OV_UINT j = 0; j < componentIDs.veclen; j++){
					resultStatements[j].value = NULL;
					resultStatements[j].veclen = 0;
					pDBWrapperVTable->m_getFittingStatements(pDBWrapper, table, componentIDs.value[j], query, searchStatementSize, &resultStatements[j]);
				}
				break;
			}
		}
	}



	for (OV_UINT i = 0; i < searchStatementSize; i++){
		Ov_SetDynamicVectorLength(&query[i].column, 0, STRING);
		Ov_SetDynamicVectorLength(&query[i].value, 0, STRING);
		Ov_SetDynamicVectorLength(&query[i].value_relation, 0, STRING);
		Ov_SetDynamicVectorLength(&query[i].value_optional, 0, STRING);
		Ov_SetDynamicVectorLength(&query[i].value_optional_relation, 0, STRING);
		Ov_SetDynamicVectorLength(&table, 0, STRING);
	}
	free(query);

	if (componentIDs.veclen == 0){
		ov_string_setvalue(errorMessage, "no component found by your statements");
		ov_string_setvalue(JsonOutput, "\"body\":{}");
		goto FINALIZE;
	}

	components = malloc(sizeof(struct component)*componentIDs.veclen);
	Ov_SetDynamicVectorLength(&endpointStruct, 0, STRING);
	OV_STRING tmpFields[2] = {"ProtocolType","EndpointString"};
	OV_STRING whereFields = NULL;
	OV_STRING tmpValues = NULL;
	for (OV_UINT i = 0; i < componentIDs.veclen; i++){
		components[i].componentID = NULL;
		ov_string_setvalue(&components[i].componentID, componentIDs.value[i]);
		components[i].endpoints = NULL;
		components[i].endpointsSize = 0;
		// Find ComponentID by Tags
		whereFields = NULL;
		ov_string_setvalue(&whereFields,"ComponentID");
		tmpValues = NULL;
		ov_string_print(&tmpValues, "%s", components[i].componentID);
		OV_INSTPTR_openAASDiscoveryServer_DBWrapper pDBWrapper = NULL;
		OV_VTBLPTR_openAASDiscoveryServer_DBWrapper pDBWrapperVTable = NULL;
		for (OV_UINT j = 0; j < pinst->v_DBWrapperUsed.veclen; j++){
			pDBWrapper = Ov_DynamicPtrCast(openAASDiscoveryServer_DBWrapper, ov_path_getobjectpointer(pinst->v_DBWrapperUsed.value[j], 2));
			if (!pDBWrapper)
				break;

			Ov_GetVTablePtr(openAASDiscoveryServer_DBWrapper,pDBWrapperVTable, pDBWrapper);
			pDBWrapperVTable->m_selectData(pDBWrapper, "Endpoints", tmpFields, 2, &whereFields, 1, &tmpValues, 1, &endpointStruct);

			if (endpointStruct.veclen > 0){
				components[i].endpointsSize = endpointStruct.veclen / 2;
				components[i].endpoints = malloc(sizeof(struct endpoint)*endpointStruct.veclen / 2);
				for (OV_UINT k = 0; k < endpointStruct.veclen/2; k++){
					components[i].endpoints[k].protocolType = NULL;
					ov_string_setvalue(&components[i].endpoints[k].protocolType, endpointStruct.value[k*2]);
					components[i].endpoints[k].endpointString = NULL;
					ov_string_setvalue(&components[i].endpoints[k].endpointString, endpointStruct.value[k*2+1]);
				}
				Ov_SetDynamicVectorLength(&endpointStruct, 0, STRING);
				break;
			}
		}
		ov_string_setvalue(&tmpValues, NULL);
		ov_string_setvalue(&whereFields, NULL);
	}
	// Generate Response
	OV_UINT stringLength = 0;
	stringLength += strlen("\"body\":{\"components\":[");
	for (OV_UINT i = 0; i < componentIDs.veclen; i++){
		stringLength += strlen("{\"componentID\":\"\", \"endpoints\":[") + strlen(components[i].componentID);
		for (OV_UINT j = 0; j < components[i].endpointsSize; j++){
			stringLength += strlen("{\"protocolType\":\"\", \"endpointString\":\"\"}") + strlen(components[i].endpoints[j].protocolType) + strlen(components[i].endpoints[j].endpointString);
			if (j < components[i].endpointsSize - 1)
				stringLength += strlen(",");
		}
		stringLength += strlen("],\"statements\":[");
		if (ov_string_compare(responseMode, "full") == OV_STRCMP_EQUAL){
			for (OV_UINT j = 0; j < resultStatements[i].veclen; j++){
				stringLength += strlen("{}") + strlen(resultStatements[i].value[j]);
				if (j < resultStatements[i].veclen - 1)
					stringLength += strlen(",");
			}
		}
		stringLength += strlen("]}");
		if (i < componentIDs.veclen - 1)
			stringLength += strlen(",");
	}
	stringLength += strlen("]}");

	*JsonOutput = ov_database_malloc(stringLength+1);
	OV_STRING pc = *JsonOutput;
	pc += sprintf(pc, "\"body\":{\"components\":[");
	for (OV_UINT i = 0; i < componentIDs.veclen; i++){
		pc += sprintf(pc, "{\"componentID\":\"%s\", \"endpoints\":[", components[i].componentID);
		for (OV_UINT j = 0; j < components[i].endpointsSize; j++){
			pc += sprintf(pc, "{\"protocolType\":\"%s\", \"endpointString\":\"%s\"}", components[i].endpoints[j].protocolType, components[i].endpoints[j].endpointString);
			if (j < components[i].endpointsSize - 1)
				pc += sprintf(pc, ",");
		}
		pc += sprintf(pc, "]");
		pc += sprintf(pc, ",\"statements\":[");
		if (ov_string_compare(responseMode, "full") == OV_STRCMP_EQUAL){
			for (OV_UINT j = 0; j < resultStatements[i].veclen; j++){
				pc += sprintf(pc, "{%s}", resultStatements[i].value[j]);
				if (j < resultStatements[i].veclen - 1)
					pc += sprintf(pc, ",");
			}
		}
		pc += sprintf(pc, "]}");
		if (i < componentIDs.veclen - 1)
			pc += sprintf(pc, ",");
	}
	pc += sprintf(pc, "]}");


	FINALIZE:
	Ov_SetDynamicVectorLength(&tokenIndex, 0, UINT);
	Ov_SetDynamicVectorLength(&endpointStruct, 0, STRING);
	ov_string_setvalue(&componentID, NULL);
	ov_string_setvalue(&securityKey, NULL);

	if (ov_string_compare(responseMode, "full") == OV_STRCMP_EQUAL){
		for (OV_UINT i = 0; i < componentIDs.veclen; i++){
			Ov_SetDynamicVectorLength(&resultStatements[i], 0, STRING);
		}
	}
	if (resultStatements)
		free(resultStatements);
	ov_string_setvalue(&responseMode, NULL);

	for (OV_UINT i = 0; i < componentIDs.veclen; i++){
		ov_string_setvalue(&components[i].componentID, NULL);
		for (OV_UINT j = 0; j < components[i].endpointsSize; j++){
			ov_string_setvalue(&components[i].endpoints[j].protocolType, NULL);
			ov_string_setvalue(&components[i].endpoints[j].endpointString, NULL);
		}
		free(components[i].endpoints);
		components[i].endpoints = NULL;
	}
	if (components)
		free(components);
	Ov_SetDynamicVectorLength(&componentIDs, 0, STRING);

	Ov_SetDynamicVectorLength(&resultStatements2, 0, STRING);

	for (OV_UINT i = 0; i < searchStatementSize; i++){
		ov_string_setvalue(&searchStatements[i].carrierID, NULL);
		ov_string_setvalue(&searchStatements[i].propertyID, NULL);
		ov_string_setvalue(&searchStatements[i].expressionSemantic, NULL);
		ov_string_setvalue(&searchStatements[i].relation, NULL);
		ov_string_setvalue(&searchStatements[i].value, NULL);
		ov_string_setvalue(&searchStatements[i].submodel, NULL);
	}
	if (searchStatements)
		free(searchStatements);

	return OV_ERR_OK;
}

OV_DLLFNCEXPORT OV_ACCESS DSServices_DSSearchServiceTypeSQLC_getaccess(
	OV_INSTPTR_ov_object	pobj,
	const OV_ELEMENT		*pelem,
	const OV_TICKET			*pticket
) {
    /*    
    *   local variables
    */

	switch(pelem->elemtype) {
		case OV_ET_VARIABLE:
			if(pelem->elemunion.pvar->v_offset >= offsetof(OV_INST_ov_object,__classinfo)) {
				if(pelem->elemunion.pvar->v_vartype == OV_VT_CTYPE)
					return OV_AC_NONE;
				else{
					if(pelem->elemunion.pvar->v_flags == 256) { // InputFlag is set
						return OV_AC_READWRITE;
					}
					/* Nicht FB? */
					if(pelem->elemunion.pvar->v_varprops & OV_VP_SETACCESSOR) {
						return OV_AC_READWRITE;
					}
					return OV_AC_READ;
				}
			}
		break;
		default:
		break;
	}
	return ov_object_getaccess(pobj, pelem, pticket);
}
